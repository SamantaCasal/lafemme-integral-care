...
const StagesSection = () => {
  ...
  const stages = [
    {
      icon: Baby,
      titleKey: "stages.pregnancy",
      descKey: "stages.pregnancy_desc",
      link: "/especialidades",
    },
    {
      icon: Heart,
      titleKey: "stages.postpartum",
      descKey: "stages.postpartum_desc",
      link: "/especialidades",
    },
    {
      icon: Flower2,
      titleKey: "stages.menopause",
      descKey: "stages.menopause_desc",
      link: "/especialidades",
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {stages.map((stage, i) => (
            ...
            >
              ...
              <h3 className="text-2xl font-semibold text-foreground">{t(stage.titleKey)}</h3>
              <p className="text-muted-foreground leading-relaxed">{t(stage.descKey)}</p>
              <Button asChild variant="link" className="text-primary">
                <Link to={stage.link}>{t("stages.view_services")}</Link>
              </Button>
            </div>
          ))}
        </div>
