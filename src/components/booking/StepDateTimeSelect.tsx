...
import { format, addDays, startOfDay, getDay, parse, addMinutes } from "date-fns";
...
const StepDateTimeSelect = ({ form, updateForm, onNext, onBack }: Props) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [tRes, eRes, bRes] = await Promise.all([
        supabase.from("availability_templates").select("*").eq("professional_id", form.professional!.id).eq("is_active", true),
        supabase.from("availability_exceptions").select("*").eq("professional_id", form.professional!.id),
        supabase.from("bookings").select("booking_date, booking_time").eq("professional_id", form.professional!.id).eq("status", "confirmed"),
      ]);
  ...
  const timeSlots = useMemo(() => {
    ...
    const customException = exceptions.find((e) => e.exception_date === dateStr && !e.is_blocked && e.start_time && e.end_time);
    const slots: string[] = [];
    ...
    const generateSlots = (startStr: string, endStr: string, slotDuration: number) => {
      ...
      while (addMinutes(current, duration) <= end) {
        ...
        const isBooked = bookedSlots.some((b) => b.booking_date === dateStr && b.booking_time === timeStr + ":00");
    ...
    if (customException) {
      ...
      dayTemplates.forEach((tmpl) => generateSlots(tmpl.start_time, tmpl.end_time, tmpl.slot_duration_minutes));
    }
    return slots;
  ...
  return (
    <div>
      <h2 className="font-serif text-2xl md:text-3xl font-bold text-foreground mb-2">Elegí día y horario</h2>
      <p className="text-muted-foreground mb-1">Seleccioná la fecha y el horario que más te convenga.</p>
      <p className="text-xs text-muted-foreground mb-6">Hora local (Paraguay — UTC-4)</p>
            ...
            {form.date ? (
              timeSlots.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground text-sm">No hay horarios disponibles para este día. Probá otro día.</p>
                </div>
              ) : (
                <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {morningSlots.map((slot) => (
                          <button key={slot} onClick={() => updateForm({ time: slot })} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${form.time === slot ? "bg-primary text-primary-foreground" : "bg-muted text-foreground hover:bg-accent"}`}>{slot}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  {afternoonSlots.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-muted-foreground mb-2">Tarde</p>
                      <div className="flex flex-wrap gap-2">
              ...
              <div className="text-center py-8">
                <p className="text-muted-foreground text-sm">Seleccioná un día en el calendario para ver los horarios.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button onClick={onNext} disabled={!form.date || !form.time} size="lg">Continuar</Button>
      </div>
    </div>
  );
};
